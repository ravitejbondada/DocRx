...
export function renderLayout(container) {
    ...
    <div class="app-layout">
      ...
      <aside class="sidebar" id="sidebar">
        ...
        <nav class="sidebar-nav">
          ...
          <button class="nav-item ${getCurrentRoute() === '/analytics' ? 'active' : ''}"
                  data-route="/analytics"
                  onclick="window.__navigate('/analytics')">
            ${NAV_ITEMS[4].icon}<span>Analytics</span>
          </button>
        ...
        <div class="sidebar-footer">
          <div class="sidebar-theme-picker">
            <div class="theme-segmented-control">
              <button class="theme-segmented-btn" id="theme-btn-dark" onclick="window.__setTheme('dark', event)" title="Switch to Dark Mode">
              ...
              <button class="theme-segmented-btn" id="theme-btn-light" onclick="window.__setTheme('light', event)" title="Switch to Light Mode">
                <svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364-6.364l-.707.707M6.364 17.636l-.707.707M6.364 6.364l.707.707M17.636 17.636l.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
                <span>Light</span>
              </button>
            </div>
          </div>
  ...
  window.__setTheme = (theme, event) => {
    ...
    localStorage.setItem('docrx_theme', theme);
    window.__updateThemeUI();
    window.dispatchEvent(new CustomEvent('docrx-theme-changed', { detail: { theme } }));
  };

  window.__toggleTheme = (event) => {
    if (event) event.stopPropagation();
    const isCurrentlyLight = document.documentElement.classList.contains('light-theme');
    window.__setTheme(isCurrentlyLight ? 'dark' : 'light');
  };
  ...
  const STATUS_ICONS = {
    syncing: `<svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 1 1 21.21 8H18" /></svg>`,
